...
int main(int argc, char* const argv[]){
  ...
  if(argc<4){
    ...
    cout<<"[2] the WS file"<<endl;
    cout<<"[3] option to use SS time dep scaling"<<endl;
    cout<<"[4-14] the fit models to use in each bin. (ordered)"<<endl;
    cout<<"  Hint, use *, positive first, then negative"<<endl;
    cout<<"********************************************"<<endl;
    return 0;
  }
  ...
  TFile *f1 =TFile::Open(argv[1]);//RS
  TFile *f2 = TFile::Open(argv[2]);//WS
  bool useTimeDepSS =atoi(argv[3]);
  TFile *f_fits[10];
  //positive fits
  for(int i=0;i<10;++i){
    f_fits[i] = TFile::Open(argv[4+i]);
    cout<<"got fit "<<argv[4+i]<<endl;
    //f_fits[i]->ls();
  }
  //return 0;
  //f2->ls();
  ...
  //
  TString channelFromFile[10];
  for(int i=0; i<5;++i){
    channelFromFile[i] = Form("RS_dst_mass_pos_bin%dw",i+1);
    channelFromFile[i+5] = Form("RS_dst_mass_neg_bin%dw",i+1);
  }
  ...
  RooWorkspace * w[10];
  for(int i=0; i<10;++i){
    w[i]= (RooWorkspace*)f_fits[i]->Get(channelFromFile[i]);
    //w[i]->Print("V");
  }
  for(auto chan : channelFromFile){
  ...
  std::vector< array<double,4> > thePars;
  for(int i=0; i<10;++i){
    array<double, 4> tmp ={(w[i]->var("kappa")->getVal()),
		   (w[i]->var("n")->getVal()),
		   (w[i]->var("nbkg")->getVal()),
		   (w[i]->var("nsig")->getVal())};
    thePars.push_back(tmp);
  }
  cout<<"pars are"<<endl;
  for( auto par: thePars){
    for( auto midpar : par){
      cout<<midpar<<",";
    }
    cout<<endl;
  ...
  for(int i=0;i<nbins;++i){
    ...
    neg_bins[i]->Sumw2();
    //WS
    pos_binsWS.push_back((TH1D*)f2->Get(Form("WS_dst_mass_td0_pos_bin%d",i+1)));
    pos_binsWS[i]->Sumw2();
    neg_binsWS.push_back((TH1D*)f2->Get(Form("WS_dst_mass_td0_neg_bin%d",i+1)));
    neg_binsWS[i]->Sumw2();
  }
  ...
  for(int i=0; i<nbins;++i){
    ...
    //WS
    pos_binsWS[i]->Add((TH1D*)f2->Get(Form("WS_ss_dst_mass_td0_pos_bin%d",i+1)),-sf);
    neg_binsWS[i]->Add((TH1D*)f2->Get(Form("WS_ss_dst_mass_td0_neg_bin%d",i+1)),-sf);
  ...
  for(int i=0;i<nbins;++i){
    theFitspos[i] = new massFit(Form("RS_dst_mass_pos_bin%d",i+1),"j3g",w[i],"FinalExtraction");
    theFitspos[i]->setData(pos_bins[i]);
    //    theFitspos[i]->initValsByHand(thePars[i]);
    theFitspos[i]->fit();
  ...
  for(int i=0;i<nbins;++i){
    theFitsneg[i] = new massFit(Form("RS_dst_mass_neg_bin%d",i+1),"j3g",w[i+5],"FinalExtraction");
    theFitsneg[i]->setData(neg_bins[i]);
    //    theFitsneg[i]->initValsByHand(thePars[i+5]);
    theFitsneg[i]->fit();
  ...
  for(int i=0;i<nbins;++i){
    theFitsposWS[i] = new massFit(Form("WS_dst_mass_pos_bin%d",i+1),"j3g",w[i],"FinalExtraction");
    theFitsposWS[i]->setData(pos_binsWS[i]);
    //    theFitsposWS[i]->initValsByHand(thePars[i]);
    theFitsposWS[i]->fit();
  ...
  for(int i=0;i<nbins;++i){
    theFitsnegWS[i] = new massFit(Form("WS_dst_mass_neg_bin%d",i+1),"j3g",w[i],"FinalExtraction");
    theFitsnegWS[i]->setData(neg_binsWS[i]);
    //    theFitsnegWS[i]->initValsByHand(thePars[i]);
    theFitsnegWS[i]->fit();
